...
class MyPipeline(DiffusionPipeline):
    ...
    def get_null_condition(self, num_images_per_prompt=None):
        
        image_embeddings = torch.zeros(1, 1, 1024).to(self.device)
        # duplicate image embeddings for each generation per prompt, using mps friendly method
        # khanh: only used for inference
    ...
    def encode_clip(self, image):
        dtype = next(self.image_encoder.parameters()).dtype

        if not isinstance(image, torch.Tensor):
            image = self.feature_extractor(images=image, return_tensors="pt").pixel_values
        else:
            image = self.feature_extractor(images=image, return_tensors="pt", do_rescale=False).pixel_values
    ...
    def train_step(self, blurred, image, masked, depth):
        
        # khanh: encode conditional image
        image_latents = self.encode_vae(image)
        masked_latents = self.encode_vae(masked)
        encoder_hidden_states = self.get_null_condition()
        
        # khanh: encode input image (original)
        latents = self.encode_vae(blurred)
        ...
        print(latent_model_input.shape, encoder_hidden_states.shape)
        print(latent_model_input.dtype, encoder_hidden_states.dtype)
        exit()
...
if __name__ == "__main__":
    ...
    loss = pipe.train_step(blurred, image, masked, depth)
    print(loss)
    noise = pipe(image, masked, depth)['images'][0]
    print(noise.size)
